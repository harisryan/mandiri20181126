...
echo 'fetching posts...';
try {
	$rows = $dbh->query('SELECT meta_id, meta_value FROM ' . POST_TABLE);
}
catch(PDOException $e) {
	echo $e->getMessage();
}
...
foreach($rows->fetchAll(PDO::FETCH_NUM) as $row) {
	...
	if( false !== $start = mb_strpos($title, START_MARKER) AND false !== $limit = mb_strpos($title, END_MARKER, $start) )
		$tobecleansed[$id]['meta_value'] = mb_substr($title, $start + strlen(START_MARKER), $limit - $start - strlen(START_MARKER));
}
...
foreach($tobecleansed as $id=>$post) {
	...
	try {
		$stmt = $dbh->prepare("UPDATE " . POST_TABLE . " SET $update WHERE meta_id = :id");
		$stmt->execute($vals);
