...
if($tax != '' && $keyword == ''){
	$args = array( 'post_type' => "rumah_sakit", 'orderby' => 'title', 'order' => ASC, 'nopaging' => true, 'direktori_entity' => $tax );
	//$query = new WP_Query( array( 'post_type' => $type, 'orderby' => 'title', 'order' => ASC, 'nopaging' => true, 'direktori_entity' => $tax ));
}elseif($tax != '' && $keyword != ''){
	$args = array( 'post_type' => "rumah_sakit", 'orderby' => 'title', 'order' => ASC, 'nopaging' => true, 's' => $keyword, 'direktori_entity' => $tax );
	...
	$args = array( 'post_type' => "rumah_sakit", 'orderby' => 'title', 'order' => ASC, 'nopaging' => true, 'direktori_wilayah' => $wilayah );
}elseif($wilayah != '' && $keyword != '' && $t!=''){ 
	$args = array( 'post_type' => 'rumah_sakit', 'orderby' => 'title', 'order' => ASC, 'nopaging' => true, 's' => $keyword, 'direktori_wilayah' => $wilayah );
}
...
if ( $query->have_posts() ) : 
	...
	while ( $query->have_posts() ) : $query->the_post(); 
			...
			$wpurl = site_url();
			array_push($output, array('latLng'=>$coord,'data'=>$data, 'options'=>array('icon'=>$wpurl.'/wp-content/themes/axamandiri/images/marker-rs.png')));
