<?php/** * Template Name: Direktori:Bengkel*/?>
<?php get_header();?>
	...
	<div id="wrapper" class="row">
		<div id="searchdir" class="p-lr-30 p-tb-15 clearfix block relative bottom-10 bg-white-70p radius-all-5">
			<form id="caridirektori" class="m-bottom-0" name="cari-direktori" action="" method="get">
				...
				<input type="hidden" name="t" value="bengkel" />
				<input type="text" name="q" class="inputdir left m-bottom-0" value="<?php echo $keyword; ?>" placeholder="Nama Rumah Sakit, Bengkel, Kantor Cabang, Kota, Wilayah"/>
		...
		<section id="maincontent">
			<div id="direktori-wrapper" class="large-8 columns clearfix">
				...
				<?php if($_GET['q'] =='' ){?>
					<h3 class="c-blue m-bottom-0 uppercase f-24">Direktori Bengkel</h3>
...
<script type="text/javascript">
		...
		function initialize() {
			jQuery.getJSON('<?php echo site_url();?>/?page_id=3066&x=<?=$_GET['x']?>&y=<?=$_GET['y']?>&q=<?php echo $_GET['q']; ?>&tax=<?php echo $tax; ?>&t=bengkel' ,function(data){
			...
			$(".view-map").click(function(){
		     ...
		     function success(position)
		     {
		         ...
		         <?php if($_GET["x"] == null && $_GET["y"] == null): ?>
			         thisUrl="<?php echo site_url('direktori/direktori-bengkel');?>"
			         window.location.href=thisUrl+"?x="+position.coords.latitude+"&y="+position.coords.longitude+"&tax=<?=$tax?>&q=<?=$keyword?>&t=bengkel&nama_produk=<?=$nama_produk?>";
		         <?php endif; ?>
		     }
