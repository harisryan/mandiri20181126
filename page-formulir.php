<?php/** * Template Name: Layanan Nasabah: Formulir */?>
<?php get_header();?>
		...
		<section id="page-half" class="grey sections clearfix">
			<div class="large-8 columns p-all-0">
				...
				<dl class="accordion" data-accordion="">
					...
					?>
				  	<dd>
					    <a class="title-accordion <?php if($first == 0) echo "active"; ?>" href="#<?=$term->slug?>"><?=$term->name?></a>
					    <div id="<?=$term->slug?>" class="content <?php if($first == 0) echo "active"; ?>">
					    	...
					    	<ul class="pdf-list small-block-grid-1 medium-block-grid-1 large-block-grid-1 clearfix">
								...
								?>
					     		<li><a href="<?php the_field('forms_upload_file');?>" class="clearfix block white shadow-grey-3" target="_blank"><span class="icon"></span><strong><?php echo get_the_title() . ' ' . getFileDetail(get_field('forms_upload_file'));?></strong><i class="fa fa-download right"></i></a></li>
					     		<?php endwhile;?>
					    	 </ul>
					    	 <?php endif;?>
					    </div>
